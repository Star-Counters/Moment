    ...
    public class CanHearObject : Conditional
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            if (targetObject.Value == null) {
                if (usePhysics2D) {
                    objectHeard.Value = MovementUtility.WithinHearingRange2D(transform, offset.Value, audibilityThreshold.Value, hearingRadius.Value, objectLayerMask);
                } else {
                    objectHeard.Value = MovementUtility.WithinHearingRange(transform, offset.Value, audibilityThreshold.Value, hearingRadius.Value, objectLayerMask);
                }
            } else { // If the target is not null then determine if that object is within sight
                objectHeard.Value = MovementUtility.WithinHearingRange(transform, offset.Value, audibilityThreshold.Value, targetObject.Value);
            }
