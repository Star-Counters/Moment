...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class LeaderFollow : NavMeshAgentGroupMovement
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            for (int i = 0; i < agents.Length; ++i) {
                ...
                if (LeaderLookingAtAgent(i) && Vector3.SqrMagnitude(leaderTransform.position - agentTransforms[i].position) < aheadDistance.Value) {
                    navMeshAgents[i].destination = transform.position + (transform.position - leaderTransform.position).normalized * aheadDistance.Value;
                } else {
                    // The destination is the behind position added to the separation vector
                    navMeshAgents[i].destination = behindPosition + DetermineSeparation(i);
                }
            }
