...
public class FSMSystem
{
	...
	public void AddState(FSMState s)
	{
		...
		if (states.Count == 0)
		{
			states.Add(new WaitState(s.NPC));//set  the WaitForOrder to the index 0 of states.
			states.Add(new DieState(s.NPC));//set  the Die to the index 1 of states.
			states.Add(s);//set other state to the index 2 of states
			if(s.NPC is MonsterAI){
				states[0].AddTransition (Transition.WaitForOrder, StateID.Idle);//then states[0] will represents the WaitForOrder state.
				s.AddTransition (Transition.WaitForOrder, StateID.Wait);
				s.AddTransition (Transition.NoHealth, StateID.Die);
			}
			currentState = s;
			currentStateID = s.ID;
			return;
	...
	public void PerformTransition(Transition trans)
	{
		...
		if (id == StateID.NullStateID)
		{
			Debug.LogError(currentState.NPC.name+ " FSM ERROR: State " + currentStateID.ToString() +  " does not have a target state " + 
		...
		foreach (FSMState state in states)
		{
			...
			{
				//if(currentState.NPC is MonsterAI)
//					Debug.Log (currentState.NPC.name+" FSMState from " + currentState + " to " + state);
