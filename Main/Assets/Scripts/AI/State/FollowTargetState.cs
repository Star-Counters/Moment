...
public class FollowTargetState : FSMState
{
	private int currentWayPoint;
	private Transform[] waypoints;
	bool followingPlayer=false;
	private void ChangeToFollowingPlayer(){
//		Debug.Log("ChangeToFollowingPlayer");
		endReachedDistance=BaseAI.nearDistance;
		followingPlayer=true;
		targetFollow=PlayerController.Instance.transform;
	}
	private void ChangeToFollowingPath(){
		//Debug.Log("ChangeToFollowingPath");
		endReachedDistance=0.2f;
		followingPlayer=false;
		targetFollow=waypoints[currentWayPoint];
	}
	public FollowTargetState(BaseAI inNPC,Transform[] wp) 
	{
		npc=inNPC;
		waypoints = wp;
		currentWayPoint = 0;
...
//				Debug.Log(currentWayPoint);
	...
	public void UpdatePath () {
		...
		seeker.StartPath (tr.position, targetFollow.position);
	}
	/*
	...
	public void OnDrawGizmos () {
		if (drawGizmos) {
			if (raycastingForGroundPlacement) {
				...
				Gizmos.DrawLine (tr.position, tr.position+Vector3.up*centerOffset);
				Gizmos.DrawLine (tr.position + Vector3.left*0.1f, tr.position + Vector3.right*0.1f);
				Gizmos.DrawLine (tr.position + Vector3.back*0.1f, tr.position + Vector3.forward*0.1f);
			}
